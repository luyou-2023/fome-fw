...
/*
 * [单定时器执行器] SingleTimerExecutor.cpp
 * 将1MHz硬件定时器与事件队列结合，提供微秒级精度的任务调度。
 * ChibiOS内核节拍在此精度下无法满足喷油/点火需求，故绕过OS直接操作硬件。
 * 调度开销实测约为CPU的5%（TIM5中断~1500Hz），属可接受范围。
 * 未来可评估迁移至ChibiOS无节拍模式以减少耦合。
 */
