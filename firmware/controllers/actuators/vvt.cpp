 ...
 * @file vvt.cpp
 * [VVT控制]
 * 可变气门正时控制,通过机油控制阀(OCV)调节凸轮轴相位,
 * 在不同工况下优化气门开启/关闭正时。
 *
 * 典型策略:
 *   - 低速/低负荷: 进气门早关 → 减少气门重叠 → 怠速稳定
 *   - 中速/中负荷: 进气门晚关 → 进气惯性充气 → 提高扭矩
 *   - 高速/高负荷: 排气门提前开 → 排气充分 → 功率提高
 *   - 部分负荷:  内部EGR → 减少泵气损失 → 省油
 *
 * 控制: PID闭环,目标角度来自3D表(RPM × 负荷)
 * 反馈: 凸轮轴传感器检测实际相位(曲轴参考角度差)
 *
 * @date Jun 26, 2016
