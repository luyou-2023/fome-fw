 ...
 * [电子节气门控制]
 * 核心控制架构:
 *   踏板传感器(冗余) → 目标位置计算 → 开环前馈(查表) + 闭环PID → H桥电机
 *
 * 安全设计:
 * - 冗余传感器: TPS主+副, PPS主+副
 * - 交叉校验: 传感器读数必须在比例范围内
 * - 跛行模式: 检测到故障时断开电机电源,节气门靠弹簧回到安全位置
 * - 卡滞检测: 持续误差超阈值判定为机械故障
 * - Lua禁用: Lua脚本可强制禁用ETB控制
 *
 * 控制算法:
 * - 前馈: etbBiasBins/etbBiasValues表,补偿弹簧力和气动力
 * - PID: 标准PID控制,带积分分离和抗积分饱和
 * - 自整定: Åström-Hägglund继电反馈+Ziegler-Nichols
 * - 自校准: TPS全开/全关位置自动标定
 *
