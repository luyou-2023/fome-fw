 ...
 * [怠速控制]
 * 控制怠速时的进气量(IAC阀或ETB节气门微开)和点火定时,
 * 在各种负载干扰(AC/发电机/风扇)下维持目标怠速转速。
 *
 * 双PID架构:
 * 1. RPM PID (idleRpmPid): 通过调节进气量控制怠速RPM
 * 2. 定时PID (idleTimingPid): 通过微调点火提前角快速响应
 *    点火调整响应快(一个发动机周期内)但范围小
 *    IAC调整响应慢(数百ms)但范围大
 *
 * 状态机:
 *   Cranking → CrankToIdleTaper → Idling ↔ Coasting / Running
 *
