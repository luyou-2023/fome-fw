 ...
 * [加速补偿]
 * 检测节气门开度(TPS)变化率,在加速时额外增加燃油,减速时减少燃油。
 *
 * 问题: 快速踩下油门时:
 *   - 空气立即增加(节气门打开)
 *   - 燃油因壁膜延迟跟不上 → 混合气瞬时变稀 → 发动机喘振/无力
 * 解决方案: TPS加速补偿在检测到TPS快速增加时,立即注入额外燃油。
 *
 * 三种策略:
 * 1. MAP变化率修正 - 通过进气压力变化检测负载突变
 * 2. TPS变化率修正 - 检测节气门开启速度
 * 3. 壁膜模型修正 - 基于wall_fuel.cpp的物理模型
 *
 * 分段补偿:
 *   将额外燃油分到多个发动机周期中逐步注入,
 *   模拟化油器的加速泵效果,避免单次大量燃油导致过浓。
 *
