 ...
 * @file wall_fuel.cpp
 * [壁膜燃油模型]
 * 模拟燃油喷射到进气道后在壁面沉积和蒸发的动态过程。
 * 基于SAE 810494 (Aquino) 和 SAE 1999-01-0553 (Maloney) 的经典模型。
 *
 * 物理原理:
 * - 喷油嘴喷射的燃油部分沉积在进气道壁面(油膜)
 * - 沉积的燃油随时间蒸发进入气流
 * - 加速时进气突增,壁膜蒸发跟不上 → 混合气变稀 → 需要额外补偿
 * - 减速时进气突减,壁膜继续蒸发 → 混合气变浓 → 需要减少喷油
 *
 * 参数:
 *   alpha: 每个循环残留在壁面的燃油比例(蒸发时间常数+转速的函数)
 *   beta:  喷射的燃油中撞击壁面的比例(取决于喷射位置和气流)
 *
 * 补偿公式:
 *   M_cmd = (M_desired - (1 - alpha) * M_film) / (1 - beta)
 *   其中 M_cmd 是实际需要喷射的燃油量
 *
