 ...
 * [触发信号处理中枢]
 * 处理曲轴/凸轮轴传感器硬件信号,完成触发解码、相位计算、
 * RPM更新、VVT位置检测等核心功能。
 * 是连接硬件中断和发动机控制计算的桥梁。
 *
 * 数据流:
 *   hwHandleShaftSignal (硬件中断)
 *     → handleShaftSignal (信号极性/有用性检查)
 *       → TriggerCentral::handleShaftSignal (噪声滤波/解码/RPM计算)
 *         → mainTriggerCallback (燃油/点火调度)
 *
 ...
 */
/* ===== 硬件触发信号入口 =====
 * 由EXIT中断服务函数调用(在hw_layer中配置的GPIO中断)
 * 这是触发信号进入ECU的第一道关口
 *
 * signalIndex: 0=主触发(曲轴), 1=次触发(凸轮轴或其他)
 * isRising: true=上升沿, false=下降沿
 * timestamp: ChibiOS系统时间戳(纳秒级)
 *
 * 注意: 在中断上下文中执行,需要尽可能快速
 * 如果触发信号频率太高(>10kHz),可能需要考虑中断合并
 */
...
/* ===== 触发信号处理核心 =====
 * 每次有效的触发齿信号到达时调用(硬件中断→handleShaftSignal→这里)
 * 注意: VR传感器下降沿不会进入此方法(在handleShaftSignal中过滤)
 *
 * 处理流程:
 * 1. 检查触发波形是否损坏
 * 2. 噪声滤波: isToothExpectedNow检查齿时间是否合理
 * 3. 跳过起动脉冲(triggerSkipPulses)
 * 4. 触发解码: decodeTriggerEvent确定当前相位
 * 5. 计算下一个预期相位(用于下次验证)
 * 6. 构建EnginePhaseInfo传递给下游
 * 7. 更新RPM, TDC标记
 * 8. 执行mainTriggerCallback(燃油/点火调度)
 * 9. MAP基凸轮解码(仅VVT_MAP_V_TWIN模式)
 *
 * 性能要求: 此函数在中断上下文中执行,必须快速完成
 * 所有耗时计算应在快速回调(250Hz)中完成
 */
void TriggerCentral::handleShaftSignal(TriggerEvent signal, efitick_t timestamp) {
	...
	/* 齿时间验证: 检查齿到达时间是否在预期范围内
	 * 如果拒绝此齿(噪声或加倍边缘),递增忽略计数但不做处理
	 */
	...
	/* 起动脉冲跳过: 某些ECU在上电时会产生虚假触发信号
	 * triggerSkipPulses配置要跳过的初始脉冲数
	 * 跳过这些脉冲可以避免虚假同步
	 */
	 ...
	 */
	/* 将触发轮索引扩展到发动机周期索引:
	 * 四冲程发动机需要720°,但触发轮可能只覆盖360°
	 * crankDivider将同一点的两个360°周期合并为完整的720°周期
	 */
	...
	/* 计算下一个预期齿的角度:
	 * 循环递增toothIndex直到找到不等于当前相位的下一个齿
	 * 用于isToothExpectedNow验证齿时序
	 * 注意: 环绕发动机周期(engineCycleEventCount)
	 */
	...
	/* TPS加速补偿:
	 * 在同步点(triggerIndexForListeners==0)记录TPS值用于计算
	 * 加速补偿需要每个发动机周期采样一次TPS
	 */
	...
	/* 构建发动机相位信息结构体:
	 * 包含当前/下一次的触发域和发动机域相位角度
	 * 传递给mainTriggerCallback用于燃油/点火调度
	 */
