 ...
 * [触发波形定义]
 * 定义TriggerWaveform类，这是所有触发轮形状的容器。
 * 每个触发轮形状由一系列事件(上升沿/下降沿)定义，
 * 每个事件有一个角度、边沿类型和所属的触发通道。
 *
 * 波形定义流程:
 * 1. initialize(operationMode, syncEdge) - 初始化周期类型和同步边沿
 * 2. addEvent720(angle, rising, wheel) - 添加一个事件
 * 3. setTriggerSynchronizationGap(ratio) - 设置同步间隙比
 * 4. initializeSyncPoint() - 计算同步点索引
 *
 * 多个品牌的触发模式通过独立的configure*函数构建:
 * 每种车型的曲轴/凸轮轮齿分布不同,需要不同的addEvent调用序列。
 *
 * @date Jan 20, 2014
