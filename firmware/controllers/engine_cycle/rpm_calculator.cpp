 ...
 * [RPM计算器]
 * 跟踪发动机旋转状态: 停止(STOPPED)→起动(SPINNING_UP)→运转(RUNNING)
 * RPM值每圈更新一次,基于上一圈经过的时间:
 *   RPM = 60 / 周期时间(秒) / 圈数(发动机圈)
 *
 * 状态转换:
 *   STOPPED → SPINNING_UP (收到触发事件)
 *   SPINNING_UP → CRANKING (RPM > 0)
 *   CRANKING → RUNNING (RPM > 起动机脱开转速)
 *   任何状态 → STOPPED (2秒无触发事件)
 *
 * 还有独立的"瞬时RPM"逻辑(基于每个齿的时间),但主控制循环使用圈RPM。
 *
 ...
 * We also have 'instant RPM' logic separate from this 'cycle RPM' logic.
 *
 * @date Jan 1, 2013
