 ...
 * [频率传感器]
 * 测量输入信号的频率(Hz)并转换为工程量。
 * 适用于: 车速传感器、柔性燃油传感器、涡轮转速传感器等。
 *
 * 测量方法:
 *   每次脉冲下降沿触发中断,记录时间戳
 *   频率 = 1 / (当前时间 - 上次时间)
 *   然后通过Biquad低通滤波器平滑
 *
 * 转换: 由SensorConverter子类完成
 *   FlexConverter: 50-150Hz → 0-100%乙醇含量
 *   VehicleSpeedConverter: Hz → km/h
 *   TurbochargerSpeedConverter: Hz → RPM
 *
 * 注意: onEdge()在中断上下文中调用,必须快速完成
 */
